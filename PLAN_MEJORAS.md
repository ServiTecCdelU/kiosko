...
|---|---|---|---|
| 1 | **Costo de producto usable de punta a punta** | Es el dato que ya existe y se tira a la basura. Sin esto no hay reporte de rentabilidad posible. | Agregar `costo` al mapeo de importación Excel (columna opcional) y a "Editar producto". Nueva columna "Margen %" en la tabla de stock (calculada, no guardada). |
| 2 | **Reporte de rentabilidad** | "¿Qué rubro me deja plata y cuál no?" es la pregunta que un dueño de despensa hace todas las semanas. | En `reportes-service.ts`: usar `precio_base` × cantidad vendida vs. `subtotal` real para margen bruto por producto/rubro/período. |
