...
| 16c | ⛔ **Tarjeta manual (posnet de banco/Prisma/Payway)** | Se implementó y después **se quitó del POS**: era para cobrar con un posnet sin integración de API (el cajero cobraba aparte y marcaba la venta como pagada a mano). Con el lector Point andando quedó redundante y era riesgoso: elegirlo por error registraba la venta **sin cobrar nada**. El valor `tarjeta` sigue existiendo en `PaymentMethod` y se sigue contabilizando en caja y reportes para no romper las ventas ya registradas. Si alguna vez se suma un posnet de banco, hay que volver a mostrar el botón con un nombre inequívoco. |
